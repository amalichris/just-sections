...
- Derive track length from `--step-scroll`; keep the trailing half-step plus half-viewport so the last step holds before the pin releases. That custom property is the only pacing knob — do not scatter magic viewport values.
