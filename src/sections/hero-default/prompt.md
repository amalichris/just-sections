...
- Keep the artwork at exactly one viewport height, but use that height only as the hero and content minimum. Place the documented device scale in normal flex flow directly after the subtitle, using the 64px desktop/tablet or 48px mobile gap; use `clamp(360px, 42vw, 420px)` for the device at tablet widths (768–1023px), retain `clamp(320px, 24vw, 340px)` on desktop, and retain the documented mobile clamp; let the hero expand intrinsically until that media is contained.
