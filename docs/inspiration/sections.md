...
### Aura — "What does Aura help protect me from?" *(source for `benefits-showcase`)*
Near-black section holding a **horizontal rail of eight portrait cards**, built on Webflow with Splide. Each card pairs a *problem* overlaid on the media ("Unwanted data exposure", "Unsafe websites", "Phishing, scam calls & texts", "Identity theft", "Credit and financial fraud", "Account takeovers", "Malware and viruses", "Cyberbullying and online predators") with the *feature that answers it* named underneath ("Online Data Removal", "Safe browsing", "Spam call protection", …). The card has no surface: the image is the card and the caption sits on the page background.

Measured live at three widths — card 256/380/357px, media frame 256×321 (4:5) / 380×521 (3:4) / 357×490 (3:4), gap 16/24/24, track 343 (peek) / 672 / **1120**. The desktop track is exactly our marketing landing container: 3 × 357 + 2 × 24 = 1119. The colour field is composed into each asset (natural 1439×1799, `object-fit: cover`) so the phone bleeds off the frame's bottom edge; some cards are a photograph alone with no device mockup at all. Controls are a prev/next arrow pair with `is-disabled` states, and no pagination on mobile.

**Flag — do not copy its accessibility.** Off-screen slides carry `aria-hidden="true"`, the carousel has no `role` and no `aria-label`, there is no live region, and mobile renders no pagination: cards four through eight are invisible to a screen reader.
