...
- `src/dev/` is the local development harness — the section gallery plus `DemoPage.jsx`. The gallery checks sections in isolation; the demo pages check what `ProductPage` does around them, and do it against real content: `src/dev/demo/<product>/` holds a snapshot of that product's page config and its imagery, one folder per product currently building against this library (`justejari` at `/`, `justconvert` at `/demo/justconvert`), because a section can look fine under short generic fixtures and fall apart under real copy lengths and a real photograph. Each snapshot is dev-only and will drift from its live page, which is fine — it is a rig for looking at sections, not a source of truth. Iterating on a section against a product's real content is faster here than round-tripping a tag through that product's `web/` department; port the finished config back once the section is right. `src/dev/` is excluded from the published package, so the library itself stays product-agnostic and asset-free, and it is the only place `import.meta.glob` or other Vite-specific APIs may be used.
