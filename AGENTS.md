...
- `src/dev/` is the local development harness — the section gallery plus `DemoPage.jsx`. The gallery checks sections in isolation; the demo page checks what `ProductPage` does around them, and does it against real content: `src/dev/demo/` holds a snapshot of JustEjari's page config and its imagery, because a section can look fine under short generic fixtures and fall apart under real copy lengths and a real photograph. That snapshot is dev-only and will drift from the live page, which is fine — it is a rig for looking at sections, not a source of truth. `src/dev/` is excluded from the published package, so the library itself stays product-agnostic and asset-free, and it is the only place `import.meta.glob` or other Vite-specific APIs may be used.
